...
public class FlinkSQLWordCount2 {
    public static void main(String[] args) throws Exception {
        ...
        SingleOutputStreamOperator<DataBean> mapedStream = socketSource.map(new MapFunction<String, DataBean>() {
            @Override
            public DataBean map(String value) throws Exception {
                String[] split = value.split(",");
                return DataBean.of(split[0], Integer.parseInt(split[1]));
            }
        });
        ...
        TableResult tableResult = SQLEnv.executeSql("select word, sum(counts) counts from v_data group by word");
